...
SD_Error SD_Init(void)
{
	...
	register_nvic(0, 0, 2, SDIO_IRQChannel);
...
//blksize:块大小	  
//返回值:错误状态
SD_Error SD_WriteBlock(const BYTE* buf,u32 addr,  u16 blksize)
{
	SD_Error errorstatus = SD_OK;
...
//nblks:要写入的块数
//返回值:错误状态												   
SD_Error SD_WriteMultiBlocks(const BYTE* buf,u32 addr,u16 blksize,u32 nblks)
{
	SD_Error errorstatus = SD_OK;
	u8  power = 0, cardstate = 0;
	u32 timeout=0,bytestransferred=0;
...
//cnt:扇区个数	
//返回值:错误状态;0,正常;其他,错误代码;	
u8 SD_WriteDisk(const BYTE* buf,u32 sector,u8 cnt)
{
	u8 sta=SD_OK;
	u8 n;
	if(CardType!=SDIO_STD_CAPACITY_SD_CARD_V1_1)sector<<=9;
